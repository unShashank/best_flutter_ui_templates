...
class CompleteTestList extends StatefulWidget {
  var testId,teacherId,subTestId;
  CompleteTestList(this.testId,this.teacherId,subTestId);
  @override
  _CompleteTestListState createState() => _CompleteTestListState(this.testId,this.teacherId,subTestId);
}

class _CompleteTestListState extends State<CompleteTestList> {
  CategoryType categoryType = CategoryType.ui;

  var testId,teacherId,subTestId;
  _CompleteTestListState(this.testId,this.teacherId,this.subTestId);
  int _current = 0;
  /* List imgList = [
  ...
  Future<List> getHttp()async{
    ...
    Response response = await Dio().get("https://www.jeevandangwalior.com/HanuSir/getCompleteTestName.php?teacherId=${teacherId}&testId=$testId&subTestId=$subTestId");
                              ...
                              Column(
                                ...
                                children: <Widget>[
                                      ...
                                      child: Row(
                                        ...
                                        children: <Widget>[
                                          Container(
                                            ...
                                            child: Text(batch["Name"].toString(),style:TextStyle(fontWeight: FontWeight.bold,fontSize: 20,)),
                                          ),

                                          ButtonTheme(minWidth:60,height:30,child: RaisedButton(child: Text("Start",style: TextStyle(color: Colors.white),),onPressed: (){
                                            Navigator.push(
                                              context,
                                              MaterialPageRoute<dynamic>(
                                                builder: (BuildContext context) => Test(batch["Question_id"],batch["test_time"]),
                                              ),
                                            );
                                          },))
                                        ],
                                  ...
                                  Container(
                                      ...
                                      child: Text("No. of questions: "+batch["Question_id"].split(",").length.toString(),style:TextStyle(fontSize: 16))
                                  ),
                                  Container(
                                      padding: const EdgeInsets.only(left:10.0,right:10),
                                      width:MediaQuery.of(context).size.width*.8,
                                      child: Text("Time: "+batch["test_time"]+" minutes",style:TextStyle(fontSize: 16))
                                  ),

                                  Container(
                                      padding: const EdgeInsets.only(left:10.0,right:10),
                                      width:MediaQuery.of(context).size.width*.8,
                                      child: Text("Status: "+batch["Status"],style:TextStyle(fontSize: 16))
                                  ),
                                  /*
                                  Container(
                                      padding: const EdgeInsets.only(left:10.0,right:10),
