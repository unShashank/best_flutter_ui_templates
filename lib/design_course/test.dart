...
class Test extends StatefulWidget {
  var questions,time;
  Test(this.questions,this.time);
  @override
  _TestState createState() => _TestState(this.questions,this.time);
}

class _TestState extends State<Test> {
  CategoryType categoryType = CategoryType.ui;
var questions,time;
  _TestState(this.questions,this.time);
  int _current = 0;
  /* List imgList = [
  ...
  void getHttp() async {
    try {
      Response response = await Dio().get("https://www.jeevandangwalior.com/HanuSir/getTestQues.php?text=jgjgg&question_ides=$questions");

      setState(() {
        testSet=true;
      });
      startTimer(int.parse(time));
      responseMap = await jsonDecode(response.toString());
        ...
        for(var i = 0; i < responseMap.length; i++){
         // setState(() {
          ...
          list.add(
              Padding(padding:const EdgeInsets.only(right: 15,bottom: 15),
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(
              ...
              children:<Widget>[
                Text((minutesToShow<10?'0':"")+minutesToShow.toString()+':'+(secondsToShow<10?'0':"")+secondsToShow.toString()),
              ]
          ),
          /*
          actions: <Widget>[
