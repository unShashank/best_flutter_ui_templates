...
class ChapterSheets extends StatefulWidget {
  var subjectId='',chapterId='';

  ChapterSheets(chapterId,subjectId){
    this.subjectId=subjectId;
    this.chapterId=chapterId;
  }

  @override
  _ChapterSheetsState createState() => _ChapterSheetsState(subjectId,chapterId);
}




class _ChapterSheetsState extends State<ChapterSheets> {
  CategoryType categoryType = CategoryType.ui;
  int _current = 0;
String subjectId='',chapterId='';
  _ChapterSheetsState(String subjectId,String chapterId){
    this.subjectId=subjectId;
    this.chapterId=chapterId;
  }
...
var sortedArray=[],tempArray=new List();
  ...
  Future<List> getHttp() async {
    try {
      Response response = await Dio().get("https://www.jeevandangwalior.com/HanuSir/getChapterSheets.php?subject_id="+subjectId+"&chapter_id="+chapterId);
      ...
      */
      ...
      print(map[0]["sheet"].toString().split("Sheet")[1].toString());

      tempArray=new List();
      for(var i=0;i<map.length;i++){
        tempArray.add(int.parse(map[i]["sheet"].toString().split("Sheet")[1]));
        tempArray.sort((a, b) => a.compareTo(b));
        print("tempArray[i]"+tempArray[i].toString());
      }
                ...
                if (projectSnap.data.length > 0) {
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      int chapter = projectSnap.data[index];
                      return Column(
                        children: <Widget>[
                          InkWell(
                            child: Card(
                            ...
                            onTap: (){
                              Navigator.push<dynamic>(
                                ...
                                MaterialPageRoute<dynamic>(
                                  builder: (BuildContext context) => SheetTest(chapterId,subjectId,"Sheet"+chapter.toString()),
                                ),
                              );
                            },
                          ),
